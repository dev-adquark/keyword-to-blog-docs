...
describe("evaluateFreshness", () => {
  it("is not applicable for a normal evergreen topic, regardless of grounding", () => {
    const result = evaluateFreshness(baseRequest(), goodPost(), false);
    expect(result.status).toBe("NOT_APPLICABLE");
    expect(result.failedChecks).toHaveLength(0);
  });

  it("blocks an unqualified current-state claim on a freshness-sensitive topic when it was NOT backed by live search", () => {
    ...
    const result = evaluateFreshness(request, post, false);
  ...
  it("passes the same current-state claim when the call actually returned a live web_search result", () => {
    const request = baseRequest({ keywords: ["software pricing"], topic: "software pricing" });
    const post = goodPost({
      sections: [
        { type: "body", heading: "Cost", contentMarkdown: "The tool currently costs $29 per month for most users." },
    ...
    const result = evaluateFreshness(request, post, true);
  ...
  it("does not flag a freshness-sensitive topic when the content avoids unqualified current-state claims", () => {
    const request = baseRequest({ keywords: ["software pricing"], topic: "software pricing" });
    const result = evaluateFreshness(request, goodPost(), false);
    expect(result.status).toBe("UNVERIFIED_ACCEPTABLE");
    expect(result.failedChecks).toHaveLength(0);
  });

  it("marks a freshness-sensitive topic with no current-state claims as VERIFIED_CURRENT when grounded", () => {
    const request = baseRequest({ keywords: ["software pricing"], topic: "software pricing" });
    const result = evaluateFreshness(request, goodPost(), true);
    expect(result.status).toBe("VERIFIED_CURRENT");
  });
});
